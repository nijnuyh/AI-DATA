...
def load_confocal(input_shape=None, set=None, z_depth=None):
    dir = 'D:/hjcho/confocal/BPAEC/' + set
    lr_lq_set = []
    hr_lq_set = []
    lr_hq_set = []
    hr_hq_set = []
...
class StarGAN(object):
    def __init__(self):
        ...
        self.discriminator_hq.compile(loss='mse',
                                   optimizer=optimizer,
                                   metrics=['accuracy'])
        self.discriminator_lq = self.build_discriminator(name='dis_lq')

        self.discriminator_lq.compile(loss='mse',
                                   optimizer=optimizer,
                                   metrics=['accuracy'])
    ...
    def build_vgg_hr(self, name=None):
        ...
        vgg = VGG19(include_top=False, weights="D:/hjcho/model/vgg19_weights_tf_dim_ordering_tf_kernels_notop.h5")
    ...
    def train(self, model, epochs, batch_size, sample_interval, set=None, z_depth=None):
        ...
        for epoch in range(epochs):
            ...
            if epoch > int(epochs/2):
                K.set_value(self.discriminator_hq.optimizer.lr,
                            1e-4 * (epochs - epoch) / (epochs - int(epochs/2)))
                K.set_value(self.discriminator_lq.optimizer.lr,
                            1e-4 * (epochs - epoch) / (epochs - int(epochs/2)))
                # K.set_value(self.combined.optimizer.lr,
                #             1e-4 * (epochs - epoch) / (epochs - int(epochs/2)))
                K.set_value(self.combined_hq.optimizer.lr,
                            1e-4 * (epochs - epoch) / (epochs - int(epochs / 2)))
                K.set_value(self.combined_lq.optimizer.lr,
                            1e-4 * (epochs - epoch) / (epochs - int(epochs / 2)))
            ...
            d_loss_real_hr = self.discriminator_hq.train_on_batch(imgs_hrhq_train, valid_hr)
            d_loss_fake_hr = self.discriminator_hq.train_on_batch(fake_hrhq, fake_hr)
            d_loss_hr = 0.5 * np.add(d_loss_real_hr, d_loss_fake_hr)
            d_loss_real_lr = self.discriminator_lq.train_on_batch(imgs_hrlq_train, valid_hr)
            d_loss_fake_lr = self.discriminator_lq.train_on_batch(fake_hrlq, fake_hr)
            ...
            #                                        image_features_hq, image_features_lq])
            g_loss_hq = self.combined_hq.train_on_batch([imgs_hrlq_train, imgs_hrhq_train],
                                                  [valid_hr, valid_hr,
                                                   image_features_hq,
                                                   image_features_lq,
                                                   imgs_hrhq_train])
            g_loss_lq = self.combined_lq.train_on_batch([imgs_hrlq_train, imgs_hrhq_train],
                                                  [valid_hr, valid_hr,
                                                  image_features_lq,
                                                  image_features_hq,
                                                  imgs_hrhq_train])
